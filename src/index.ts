...
import {
  bytesToiB,
  anyToBytes,
  checkRequestAndReturnRequest,
  commentsForEachIssue,
} from "./utils";
...
const allocationDatacap = async () => {
  try {
    ...
    logGeneral(
      `${config.LOG_PREFIX} 0 Number of fetched comments: ${rawIssues.length}`
    );
    const promArr = [];
    ...
    const cleanedRawIssues = rawIssues.filter(
      (issue: any) =>
        checkLabel(issue) &&
        checkRequestAndReturnRequest(requestListForEachIssue, issue).isValid
    );

    const datacapRequestedByEachClientArr = cleanedRawIssues.map(
      (issue: any) => {
        return {
          issueNumber: issue.number,
          totaldDcRequestedByClient: parseIssue(issue.body).datacapRequested,
          totaldDcRequestedByClientBytes: anyToBytes(
            parseIssue(issue.body).datacapRequested.toString()
          ),
          weeklyAllocationRequestedByClient: parseIssue(issue.body)
            .dataCapWeeklyAllocation,
          weeklyDcAllocationBytes: anyToBytes(
    ...
    const allClientsFromApi = await Promise.all(
      cleanedRawIssues.map(async (issue: any) => {
        const clientAllowanceObj = await axios({
          method: "GET",
          url: `${config.filpusApi}/getAllowanceForAddress/${
            parseIssue(issue.body).address
          }`,
          headers: {
            "x-api-key": config.filplusApiKey,
          },
        ...
        if (!clientAllowanceObj?.data || !clientAllowanceObj.data.allowance) {
          ...
          if (parseIssue(issue.body).address.startsWith("f1")) {
            actorAddress = await api.actorAddress(
              parseIssue(issue.body).address
            );
          } else {
          ...
          if (!checkClient[0]) {
            logWarn(
              `${config.LOG_PREFIX} ${issue.number} - It looks like the client has 0B datacap remaining.`
            );
    ...
    const allClientsFromApiCleaned = allClientsFromApi.filter(
      (item: any) => item.dataCapRemainingBytes !== -1
    );
    ...
    for (const issue of cleanedRawIssues) {
      ...
      const requestList = requestListForEachIssue.find(
        (requestItem: any) => requestItem.issueNumber === issue.number
      ).requestList;
      ...
      if (!client) {
        logGeneral(
          `${config.LOG_PREFIX} ${issue.number} skipped --> dc not allocated yet`
        );
        continue;
      }
      ...
      const lastRequestDataCapAllocatedConvert =
        lastRequest.allocationDatacap.endsWith("B")
      ...
      const dataCapRemainingBytes = allClientsFromApiCleaned.find(
        (item: any) => item.issueNumber === issue.number
      ).dataCapRemainingBytes;
      ...
      logGeneral(
        `${config.LOG_PREFIX} ${
          issue.number
        } datacap remaining / datacp allocated: ${(margin * 100).toFixed(2)} %`
      );

      const dcAllocationRequested = calculateAllocationToRequest(
        requestNumber,
        totalDcGrantedForClientSoFar,
        datacapRequestedByEachClientArr.find(
          (item: any) => item.issueNumber === issue.number
        ...
        new Promise<void>(async (resolve, reject) => {
          try {
            ...
            const info: IssueInfo = {
              ...
              totalDcGrantedForClientSoFar: bytesToiB(
                totalDcGrantedForClientSoFar
              ),
              totaldDcRequestedByClient: datacapRequestedByEachClientArr.find(
                (item: any) => item.issueNumber === issue.number
            ...
            if (issue.number === 84) {
              ...
              logGeneral(
                `CREATE REQUEST COMMENT ${config.LOG_PREFIX} ${info.issueNumber}`
              );
              ...
              logGeneral(
                `${config.LOG_PREFIX} ${issue.number}, posted subsequent allocation comment.`
              );
    ...
    logGeneral(
      `${config.LOG_PREFIX} 0 Subsequent-Allocation-Bot ended. ${
        issueInfoList.length ? issueInfoList.length : 0
      } issues commented`
    );
    logGeneral(
      `${
        config.LOG_PREFIX
      } 0 Subsequent-Allocation-Bot - commented issues number: ${issueInfoList.map(
        (info: any) => info.issueNumber
      )}, ${issueInfoListClosed.map((num: any) => num)}`
    );
    logGeneral(
      `${
        config.LOG_PREFIX
      } 0 Subsequent-Allocation-Bot - issues reaching the total datacap: ${issueInfoListClosed.map(
        (num: any) => num
      )}`
    );
  } catch (error) {
      ...
      promArr.push(
        new Promise<void>(async (resolve, reject) => {
          ...
          const githubHandles = addresses.map(
            (address: string) =>
              notaries.find(
                (notar: any) => notar.ldn_config.signing_address === address
              ).github_user[0]
          );
          ...
          logGeneral(
            `Posted stats comment, ${config.LOG_PREFIX} ${info.issueNumber}`
          );
          resolve();
        })
      );
...
const calculateAllocationToRequest = (
  ...
  logDebug(
    `${
      config.LOG_PREFIX
    } ${issueNumber} weekly datacap requested by client: ${bytesToiB(
      weeklyDcAllocationBytes
  ...
  logDebug(
    `${
      config.LOG_PREFIX
    } ${issueNumber} total datacap requested by client: ${bytesToiB(
      totaldDcRequestedByClient
  ...
  switch (requestNumber) {
    ...
    case 2: //3rd req - 200% of the amount in the issue
      condition =
        weeklyDcAllocationBytes * 2 <= totaldDcRequestedByClient * 0.2;
      nextRequest = condition
        ? weeklyDcAllocationBytes * 2
        : totaldDcRequestedByClient * 0.2;
      rule = condition
    ...
    case 3: //4th req - 400% of the amount in the issue
      condition =
        weeklyDcAllocationBytes * 4 <= totaldDcRequestedByClient * 0.4;
      nextRequest = condition
        ? weeklyDcAllocationBytes * 4
        : totaldDcRequestedByClient * 0.4;
      rule = condition
    ...
    default:
      ...
      condition =
        weeklyDcAllocationBytes * 8 <= totaldDcRequestedByClient * 0.8;
      nextRequest = condition
        ? weeklyDcAllocationBytes * 8
        : totaldDcRequestedByClient * 0.8;
      rule = condition
  ...
  const sumTotalAmountWithNextRequest =
    nextRequest + totalDcGrantedForClientSoFar;
  logDebug(
    `${
      config.LOG_PREFIX
    } ${issueNumber} sumTotalAmountWithNextRequest (sum next request + total datcap granted to client so far): ${bytesToiB(
  ...
  if (sumTotalAmountWithNextRequest > totaldDcRequestedByClient) {
    logDebug(
      `${config.LOG_PREFIX} ${issueNumber} sumTotalAmountWithNextRequest is higher than total datacap requested by client (${totaldDcRequestedByClient}, requesting the difference of total dc requested - total datacap granted so far)`
    );
  ...
  if (nextRequest <= 0) {
    logDebug(
      `${config.LOG_PREFIX} ${issueNumber} - seems that the client reached the total datacap request in this issue. This should be checked and closed`
    );
  ...
  logDebug(
    `${config.LOG_PREFIX} ${issueNumber} nextRequest ${bytesToiB(nextRequest)}`
  );
